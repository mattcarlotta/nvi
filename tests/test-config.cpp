#include "config.h"
#include "options.h"
#include "gtest/gtest.h"
#include <string>
#include <vector>

nvi::Config config("staggered_with_comments", "../envs");
nvi::options_t opts = config.generate_options()->get_options();

TEST(Config, parseable_debug) { EXPECT_EQ(opts.debug, true); }

TEST(Config, parseable_directory) { EXPECT_EQ(opts.dir, "path/to/custom/directory"); }

TEST(Config, parseable_env) { EXPECT_EQ(opts.environment, "my_env"); }

TEST(Config, parseable_execute) { EXPECT_EQ(opts.commands.size(), 3); }

TEST(Config, parseable_files) {
    const std::vector<std::string> files = {"test1.env", "test2.env", "test3.env"};
    EXPECT_EQ(opts.files, files);
}

TEST(Config, parseable_project) { EXPECT_EQ(opts.project, "my_project"); }

TEST(Config, parseable_required_envs) {
    const std::vector<std::string> envs = {"TEST1", "TEST2", "TEST3"};
    EXPECT_EQ(opts.required_envs, envs);
}
