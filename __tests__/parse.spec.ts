...
describe('Parse Method', () => {
    ...
    it('parses (\\r) line endings', () => {
        ...
        const RPayload = parse(Buffer.from('SERVER=localhost\rPASSWORD=password\rDB=tests\r'));
        expect(RPayload).toEqual(expectedPayload);

        const NPayload = parse(Buffer.from('SERVER=localhost\nPASSWORD=password\nDB=tests\n'));
    ...
    it('parses single command-line substitutions', () => {
        let result = parse(
            Buffer.from("MESSAGE=$(echo 'Welcome To The Mad House' | sed 's/[^A-Z]//g')")
        );
    ...
    it("extends envs that contain '# extends path/to/.env'", () => {
        const result = parse(Buffer.from('# extends: tests/.env.extends2\nEXTENDED=true'));
    ...
    it("doesn't parse envs that contain invalid extension paths", () => {
        const result = parse(Buffer.from('# extends: tests/.env.invalid\nEXTENDED=false'));
