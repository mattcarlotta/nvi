...
static void find_env_candidates(arena_t *scratch, const file_details_t *file, const file_ext_t *file_ext_match,
    ...
    for (size_t acc_idx = 0; acc_idx < file_ext_match->accessor_count; ++acc_idx) {
        ...
        while (search_start + acc->prefix_len <= file->len) {
            ...
            DYN_ARR_APPEND(scratch, env_candidate_list, ((env_candidate_t){.pos = match_pos, .acc = acc_idx}));
        }
    }
}
...
void scan_file_content(arena_t *scratch, const file_details_t *file, const file_ext_t *file_ext_match,
    ...
    find_env_candidates(scratch, file, file_ext_match, &env_candidates);
